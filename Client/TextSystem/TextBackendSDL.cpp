...
public:
	...
	bool GetGlyphMetrics(FontHandle font, uint32_t codepoint, GlyphMetrics& outMetrics) override
	{
		...
		if (!ok) {
			// 대체 방법: 렌더링된 서피스를 이용해 메트릭을 근사합니다
			...
			minx = 0;
			maxx = surf->w;
			miny = -TTF_FontAscent(ttf);  // 상단 정렬로 가정합니다
			maxy = TTF_FontDescent(ttf);  // 기준선이 하단에 있다고 가정합니다
			advance = surf->w;
	...
	const Glyph* GetGlyph(FontHandle font, uint32_t codepoint, const Color& color) override
	{
		...
		if (hasMetrics) {
			...
			// bearingY는 기준선에서 렌더링된 글리프 서피스 상단까지의 거리입니다
			// 렌더링된 서피스는 글리프 전체를 포함하므로, 그 상단은 baseline - miny 위치에 있습니다
			// 따라서 bearingY는 ascent - (기준선 기준 서피스 상단 위치)가 되어야 합니다
			// miny는 음수이므로(기준선보다 위쪽), surface_top = baseline - miny = baseline + |miny|입니다
			// 그러므로 bearingY = ascent + miny 입니다
			metrics.bearingY = ascent + miny;
		} else {
			// 대체 방법: 서피스로부터 메트릭을 근사합니다
			int ascent = TTF_FontAscent(ttf);
			metrics.width = rgbaSurface->w;
			metrics.height = rgbaSurface->h;
			metrics.advance = rgbaSurface->w;
			metrics.bearingX = 0;
			metrics.bearingY = ascent;  // 상단 정렬로 가정합니다
		}
